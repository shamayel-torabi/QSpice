...
void dq_controller(struct sDQ_INVERTOR_CONTROLLER *inst){
   double Valph = (inst->Valph_k[0] + inst->Valph_k[1] + inst->Valph_k[2] + inst->Valph_k[3]) / 4.0;
   double Vbeta = (inst->Vbeta_k[0] + inst->Vbeta_k[1] + inst->Vbeta_k[2] + inst->Vbeta_k[3]) / 4.0;
   
   double Ialph = (inst->Ialph_k[0] + inst->Ialph_k[1] + inst->Ialph_k[2] + inst->Ialph_k[3]) / 4.0;
   double Ibeta = (inst->Ibeta_k[0] + inst->Ibeta_k[1] + inst->Ibeta_k[2] + inst->Ibeta_k[3]) / 4.0;
   
   double Io_alph = (inst->Ialph_k[0] + inst->Ialph_k[1] + inst->Ialph_k[2] + inst->Ialph_k[3]) / 4.0;
   double Io_beta = (inst->Ibeta_k[0] + inst->Ibeta_k[1] + inst->Ibeta_k[2] + inst->Ibeta_k[3]) / 4.0;

   double theta = inst->dsogi(Valph, Vbeta);

   double sinValue = sin(theta);
   double cosValue = cos(theta);

   double Vd =  Valph * cosValue + Vbeta * sinValue;
   double Vq = -Valph * sinValue + Vbeta * cosValue;

   double Id =  Ialph * cosValue + Ibeta * sinValue;
   double Iq = -Ialph * sinValue + Ibeta * cosValue;

   double Iod =  Io_alph * cosValue + Io_beta * sinValue;
   double Ioq = -Io_alph * sinValue + Io_beta * cosValue;

   inst->dq(inst->P, inst->Q, Id, Iq, Iod, Ioq, Vd, Vq, inst->Vdc);
