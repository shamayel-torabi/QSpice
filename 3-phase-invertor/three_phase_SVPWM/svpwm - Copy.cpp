// Automatically generated C++ file on Sat Apr 11 10:33:04 2026
//
...
void calculate_svpwm_switchtime(sSVPWM *inst, double U_alpha, double U_beta) {
   double  U_max = (1/_SQRT3)*_U_DC;
   double T_halfsample = inst->xpeak / inst->mcu_clk;

   uint8_t sector = inst->theta/_PIdiv3;
   ...
   /* Upper switches */
   uint16_t ontime_t_0_half = (t_0_half) * inst->mcu_clk ;
   uint16_t ontime_value_1 = (t_0_half + t_1) * inst->mcu_clk;
   uint16_t ontime_value_2 = (t_0_half + t_2) * inst->mcu_clk;
   uint16_t ontime_value_3 = (t_0_half + t_1 + t_2) * inst->mcu_clk;
   ...
   inst->trg_a_r = inst->trg_e + inst->switchtime[0] / inst->mcu_clk ;
   inst->trg_a_f = inst->trg_e + (2 * t_mid - inst->switchtime[0] / inst->mcu_clk);

   inst->trg_b_r = inst->trg_e + inst->switchtime[1] /  inst->mcu_clk ;
   inst->trg_b_f = inst->trg_e + (2 * t_mid - inst->switchtime[1] / inst->mcu_clk );

   inst->trg_c_r = inst->trg_e + inst->switchtime[2] / inst->mcu_clk ;
   inst->trg_c_f = inst->trg_e + (2 * t_mid - inst->switchtime[2] / inst->mcu_clk);
...
extern "C" __declspec(dllexport) void svpwm(struct sSVPWM **opaque, double t, union uData *data)
{
   ...
   double  Per  = data[2].d; // input parameter
   ...
   if(!*opaque)
   {
      *opaque = (struct sSVPWM *) malloc(sizeof(struct sSVPWM));
      ...
      calculate_svpwm_switchtime(inst, Va, Vb);
