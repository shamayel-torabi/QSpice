// Automatically generated C++ file on Sun Nov  2 12:10:39 2025
//
...
extern "C" __declspec(dllexport) void spwm(struct sSPWM **opaque, double t, union uData *data)
{
   ...
   const double          CFS     = data[3].d ; // input parameter
   ...
   if(!*opaque)
   {
      *opaque = (struct sSPWM *) malloc(sizeof(struct sSPWM));
      ...
      inst->ttol = TTOL;
      inst->mcu_clk = FREQ;
      inst->mcu_aar = (unsigned short) 50 * CFS;
      inst->xcmp = 0;

      // inst->trg1 = 2 * inst->mcu_aar / inst->mcu_clk;
      // inst->trg2 = 2 * inst->mcu_aar / inst->mcu_clk;
      // inst->trg3 = 2 * inst->mcu_aar / inst->mcu_clk;
      // inst->trg4 = 2 * inst->mcu_aar / inst->mcu_clk;

      inst->trg1 = 0;
      inst->trg2 = 1 * inst->mcu_aar / inst->mcu_clk;
      inst->trg3 = 0;
      inst->trg4 = 2 * inst->mcu_aar / inst->mcu_clk;


      inst->carrier_f = (unsigned short) CFS;
      inst->duty = 0;
      inst->counter = 1;
      inst->up_counte = true;
...
// Implement module evaluation code here:
   ...
   {
      inst->xcntr++;
      //inst->mcu_aar = (unsigned short) 100 * CFS;

      //===================================================================
      // control algorithm interrupt - START ==============================
      //===================================================================

      double theta = (inst->counter / inst->carrier_f) * (PI / 180.0);
      inst->duty = floor(0.9 * sin(theta) * inst->carrier_f);
      //===================================================================
      ...
      inst->trg2 = inst->trg4 + inst->mcu_aar / inst->mcu_clk;
      inst->trg3 = inst->trg4 + (2*inst->mcu_aar - inst->xcmp) / inst->mcu_clk;
      inst->trg4 = inst->trg4 + 2*inst->mcu_aar / inst->mcu_clk;

      inst->maxstep = inst->mcu_aar / inst->mcu_clk;
   }
