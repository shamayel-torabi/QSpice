// Automatically generated C++ file on Fri Oct 24 10:03:45 2025
//
...
extern "C" __declspec(dllexport) void rms(struct InstanceData **opaque, double t, union uData *data)
{
   ...
   const double  Is   = data[2].d; // input parameter
   double &Irms = data[3].d; // output
...
// Implement module evaluation code here:
  ...
  // clock state change triggered
   if (t >= inst->next_t) {
      ...
      v_square = (In - inst->v_mean) * (In - inst->v_mean);

      inst->v_sq_sum += v_square ;

      inst->v_mn_array[inst->index] = In;
