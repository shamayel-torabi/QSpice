...
public:
    ...
    void operator()(double vd, double vq, double theta){
        double valpha = cos(theta) * vd - sin(theta) * vq;
        double vbeta  = sin(theta) * vd + cos(theta) * vq;
        
        double Vref = hypotf(valpha, vbeta) / Vdc;
        //double theta = atan2(vbeta, valpha);v
