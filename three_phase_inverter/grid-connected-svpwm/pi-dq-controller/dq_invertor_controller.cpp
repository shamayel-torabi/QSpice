...
void dq_controller(struct sDQ_INVERTOR_CONTROLLER *inst, double t){
   ...
   dqt.Vd =  inst->Valpha * cosValue + inst->Vbeta * sinValue;
   dqt.Vq = -inst->Valpha * sinValue + inst->Vbeta * cosValue;

   dqt.Id =  inst->Ialpha * cosValue + inst->Ibeta * sinValue;
   dqt.Iq = -inst->Ialpha * sinValue + inst->Ibeta * cosValue;
