...
void dq_controller(struct sDQ_INVERTOR_CONTROLLER *inst, double t){
   ...
   double Vd =  inst->Valpa * cosValue + inst->Vbeta * sinValue;
   double Vq = -inst->Valpa * sinValue + inst->Vbeta * cosValue;

   double Id =  inst->Ialpa * cosValue + inst->Ibeta * sinValue;
   double Iq = -inst->Ialpa * sinValue + inst->Ibeta * cosValue;
