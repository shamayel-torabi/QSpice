// Automatically generated C++ file on Sat Apr 18 18:59:45 2026
//
// To build with Digital Mars C++ Compiler:
//
//    dmc -mn -WD symetric_svpwm.cpp kernel32.lib

#include <malloc.h>
#include <inttypes.h>
#include "inc/symmetrical_svpwm.h"
...
struct sSYMETRIC_SVPWM
{
   ...
   SymmetricalSVPWM pwm;
};

extern "C" __declspec(dllexport) void symetric_svpwm(struct sSYMETRIC_SVPWM **opaque, double t, union uData *data)
{
   ...
   if(!*opaque)
   {
      *opaque = (struct sSYMETRIC_SVPWM *) malloc(sizeof(struct sSYMETRIC_SVPWM));
      bzero(*opaque, sizeof(struct sSYMETRIC_SVPWM));

      struct sSYMETRIC_SVPWM *inst = *opaque;
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sSYMETRIC_SVPWM *inst, double t)
{
   return inst->maxstep; // implement a good choice of max timestep size that depends on struct sSVPWM
}

extern "C" __declspec(dllexport) void Trunc(struct sSYMETRIC_SVPWM *inst, double t, union uData *data, double *timestep)
{ // limit the timestep to a tolerance if the circuit causes a change in struct sSYMETRIC_SVPWM
   const double ttol = 1e-9; // 1ns default tolerance
   if(*timestep > ttol)
   {
      struct sSYMETRIC_SVPWM tmp = *inst;
      symetric_svpwm(&(&tmp), t, data);
      if(tmp.xcntr != inst->xcntr) // implement a meaningful way to detect if the state has changed
         *timestep = ttol;
   }
}

extern "C" __declspec(dllexport) void Destroy(struct sSYMETRIC_SVPWM *inst)
{
   free(inst);
}
